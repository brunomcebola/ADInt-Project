...
@presential_services.route("/services")
def get_services():
    req = requests.get("%s/services" % presential_services_url, headers=header)
    return req.json(), req.status_code


@presential_services.route("/services/filter")
...
@presential_services.route("/service/<service_id>", methods=["GET"])
def get_service(service_id):
    req = requests.get("%s/service/%s" % (presential_services_url, service_id), headers=header)
    return req.json(), req.status_code


@presential_services.route("/service/<service_id>", methods=["DELETE"])
def delete_service(service_id):
    req = requests.delete("%s/service/%s" % (presential_services_url, service_id), headers=header)
    return req.json(), req.status_code


@presential_services.route("/service/<service_id>/evaluations")
...
@presential_services.route("/service/create", methods=["POST"])
def create_service():
